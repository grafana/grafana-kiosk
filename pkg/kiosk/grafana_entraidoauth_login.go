...
// GrafanaKioskEntraIdOauth creates a chrome-based kiosk using a oauth2 authenticated account.
func GrafanaKioskEntraIdOauth(cfg *Config, messages chan string) {
	...
	*/
	...
	log.Println("EntraID OAuth Handler:")
	...
	if err := chromedp.Run(taskCtx,
		...
		chromedp.WaitVisible(`//input[@name="`+cfg.GoAuth.PasswordField+`" and not(@class="moveOffScreen")]`, chromedp.BySearch),
